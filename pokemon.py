...
class Pokemon():
    ...
    def print_all_pokemon():
        ...
        table_print(("Number", "Name", "Pokemon", "Type", "HP", "Owner"), pokemon_list, (10, 10, 10, 10, 10, 10))
        return 1

    # A static method to print all the non-owned pokemon
    @staticmethod
    def print_non_owned_pokemon():
        non_owned_pokemon = [pokemon for pokemon in Pokemon.__all_pokemon_list if not pokemon.get_owner() is None]
        if not non_owned_pokemon:
            print("No Pokemon without owners.")
            return []

        pokemon_list = []
        for pokemon in non_owned_pokemon:
            pokemon_list.append((pokemon.get_num(), pokemon.get_name(), pokemon.get_pokemon_type(), pokemon.get_kind(), pokemon.get_hp()))

        table_print(("Number", "Name", "Pokemon", "Type", "HP"), pokemon_list, (10, 10, 10, 10, 10))
        return non_owned_pokemon
...
def create_default():
    ...
    pokemon7 = Pokemon("Sandy", "Sandslash", "Ground", person=None)
    pokemon8 = Pokemon("Blasty", "Blastoise", "Water", person=None)
    pokemon9 = Pokemon("Weedy", "Weedle", "Bug", person=None)
    pokemon10 = Pokemon("Rai", "Raichu", "Electric", person=None)
